...
class Natura(QMainWindow):
    ...
    def setup(self, config: configparser.ConfigParser):
        ...
        if self.data["types"][0] == self.data["types"][1]:
            typelabel = QLabel()
            typePix = QPixmap("data/sprites/" +  self.data["types"][0] + ".png")
            typePix=typePix.scaledToHeight(25)
            typelabel.setPixmap(typePix)
            typeBOx.addWidget(typelabel)
        else :
            typelabel1 = QLabel()
            typePix = QPixmap("data/sprites/" +  self.data["types"][0] + ".png")
            typePix=typePix.scaledToHeight(25)
            typelabel1.setPixmap(typePix)
            typeBOx.addWidget(typelabel1)
            typelabel2 = QLabel()
            typePix = QPixmap("data/sprites/" +  self.data["types"][1] + ".png")
            typePix=typePix.scaledToHeight(25)
            typelabel2.setPixmap(typePix)
            typeBOx.addWidget(typelabel2)
        typeWidget.setLayout(typeBOx)
        label_container.addWidget(typeWidget)
